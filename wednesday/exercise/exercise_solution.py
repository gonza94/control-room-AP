...
for rf_phase_shift in [0,1,2,3,4,5]:
    ...
    #-------------------------------------
    #---- let's measure BPMs' phases after change
    #-------------------------------------
    bpm_phases_after = []
    print ("----------- BPM Phases After Change ---------")
    print (" BPM    pos[m]   Phase[deg] ")
    for bpm_model in bpm_model_nodes:
        bpm_phase = bpm_model.getPhase()
        bpm_phases_after.append(bpm_phase)
        st = " %22s  %7.3f   %+6.1f "%(bpm_model.getName(),bpm_model.getPosition(),bpm_phase)
        print (st)
    print ("---------------------------------------")

    phasechange = np.array(bpm_phases_after)-np.array(bpm_phases_init_arr)
    phasechange = -180+(phasechange+180)%(360)

    plt.plot(phasechange)
